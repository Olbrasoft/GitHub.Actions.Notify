        ...
        case "$event_type" in
            ...
            ci-complete)
                ...
                if [ "$status" = "success" ]; then
                    echo ""
                    echo "ALL CI CHECKS PASSED. This is a COMPLETE UNIT OF WORK — handle everything NOW, do NOT passively wait for any other wake event."
                    echo ""
                    echo "Step 1 — get full PR state:"
                    ...
                    echo ""
                    echo "Step 2 — act on the result (FIRST match wins):"
                    echo ""
                    echo "  IF state=MERGED → SKIP. Nothing to do."
                    echo "  IF state=CLOSED → SKIP. PR was abandoned."
                    echo "  IF mergeable=CONFLICTING → rebase or resolve conflicts, push. Wait for next ci-complete wake."
                    echo "  IF mergeable=UNKNOWN → sleep 5, re-run Step 1."
                    echo ""
                    echo "  IF copilot HAS reviewed (state is COMMENTED/APPROVED/CHANGES_REQUESTED):"
                    echo "    → Go to Step 3."
                    echo ""
                    echo "  IF copilot has NOT reviewed yet (state is null):"
                    echo "    → POLL ACTIVELY — do NOT passively wait for a wake event."
                    echo "      Copilot review takes 3-5 minutes. Run this polling loop:"
                    echo ""
                    echo "      for i in \$(seq 1 10); do"
                    echo "        sleep 60"
                    echo "        RESULT=\$(gh pr view $pr_num --repo $repo_name --json state,reviews \\"
                    echo "          --jq '{state, copilot: ([.reviews[] | select(.author.login | startswith(\"copilot-pull-request-reviewer\"))] | last | .state)}')"
                    echo "        echo \"Poll \$i/10: \$RESULT\""
                    echo "        STATE=\$(echo \"\$RESULT\" | jq -r '.state')"
                    echo "        COPILOT=\$(echo \"\$RESULT\" | jq -r '.copilot')"
                    echo "        [ \"\$STATE\" != \"OPEN\" ] && break"
                    echo "        [ \"\$COPILOT\" != \"null\" ] && break"
                    echo "      done"
                    echo ""
                    echo "      Run the loop above in Bash (timeout: ~11 minutes). When it exits:"
                    echo "      - If copilot is non-null → go to Step 3"
                    echo "      - If state is not OPEN → SKIP"
                    echo "      - If 10 iterations passed with no review → notify user, stop"
                    echo ""
                    echo "Step 3 — handle Copilot review + merge:"
                    echo "  Read BOTH the review body/state AND inline comments:"
                    echo "    REVIEW_BODY=\$(gh pr view $pr_num --repo $repo_name --json reviews \\"
                    echo "      --jq '[.reviews[] | select(.author.login | startswith(\"copilot-pull-request-reviewer\"))] | last | .body // \"\"')"
                    echo "    INLINE=\$(gh api repos/$repo_name/pulls/$pr_num/comments --jq '.[].body')"
                    echo "  IF REVIEW_BODY or INLINE comments exist: fix them in code, commit, push."
                    echo "    Then poll for CI on fix commit (same pattern: sleep 60, check gh pr checks $pr_num --repo $repo_name)."
                    echo "    When CI passes → MERGE."
                    echo "  IF no review body AND no inline comments (or APPROVED): MERGE IMMEDIATELY."
                    echo "  MERGE command:"
                    echo "    gh pr merge $pr_num --repo $repo_name --merge --delete-branch"
                    echo ""
                    echo "  Copilot reviews ONCE. After fixing + CI green → MERGE. No second review."
                    echo ""
                    echo "Step 4 — after merge:"
                    echo "  Repos WITH deploy: VirtualAssistant → wait for deploy-complete wake."
                    echo "  Repos WITHOUT deploy: cr, GitHub.Actions.Notify → notify user: done."
                    echo ""
                    echo "CRITICAL: Do NOT say 'čekám na review' or 'waiting for wake event'."
                    echo "You ARE the review checker now. Poll, act, merge. One unit of work."
                else
                    echo ""
                    echo "CI FAILED. Handle it NOW:"
                    echo "  1. Read failed logs: gh run list --repo $repo_name --branch $pr_branch --limit 1 --json databaseId --jq '.[0].databaseId'"
                    echo "     Then: gh run view <ID> --repo $repo_name --log-failed 2>&1 | tail -50"
                    echo "  2. Fix the issue in code"
                    echo "  3. Commit and push"
                    echo "  4. Wait for next ci-complete wake event (fix needs CI)"
