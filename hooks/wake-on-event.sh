        ...
        case "$event_type" in
            ...
            ci-complete)
                ...
                if [ "$status" = "success" ]; then
                    ...
                    echo "      done"
                    echo ""
                    echo "      Run the loop above in Bash (timeout: 660000). When it exits:"
                    ...
                    echo "  Read review comments:"
                    echo "    gh api repos/$repo_name/pulls/$pr_num/comments --jq '.[].body'"
                    echo "  IF comments exist: fix them in code, commit, push."
                    echo "    Then poll for CI on fix commit (same pattern: sleep 60, check gh pr checks $pr_num)."
                    echo "    When CI passes → MERGE."
                    echo "  IF no comments (or APPROVED): MERGE IMMEDIATELY."
                    echo "  MERGE command:"
                    echo "    gh pr merge $pr_num --repo $repo_name --merge --delete-branch"
                    echo ""
